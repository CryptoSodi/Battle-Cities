...
exports.TerrainTileDestroyer = void 0;
...
class TerrainTileDestroyer extends core_1.GameObject {
    constructor(argDamage) {
        const damage = Math.min(argDamage, TankBulletWallDamage_1.TankBulletWallDamage.High);
    ...
    collide(collision) {
        ...
        frontContacts.forEach((contact) => {
            const tile = contact.collider.object;
            const isBrickWall = tile.tags.includes(Tag_1.Tag.Brick);
            const isSteelWall = tile.tags.includes(Tag_1.Tag.Steel);
            // TODO: this check should be a part of bullet attributes model
            const canDestroySteelWall = this.damage === TankBulletWallDamage_1.TankBulletWallDamage.High;
    ...
    getFrontRowContacts(contacts) {
        ...
        if (rotation === Rotation_1.Rotation.Up) {
            ...
            return this.limitContactsToImpactWidth(contacts.filter((contact) => contact.box.max.y === frontY));
        }
        if (rotation === Rotation_1.Rotation.Down) {
            const frontY = Math.min(...contacts.map((contact) => contact.box.min.y));
            return this.limitContactsToImpactWidth(contacts.filter((contact) => contact.box.min.y === frontY));
        }
        if (rotation === Rotation_1.Rotation.Left) {
            const frontX = Math.max(...contacts.map((contact) => contact.box.max.x));
            return this.limitContactsToImpactWidth(contacts.filter((contact) => contact.box.max.x === frontX));
        }
        if (rotation === Rotation_1.Rotation.Right) {
            const frontX = Math.min(...contacts.map((contact) => contact.box.min.x));
            return this.limitContactsToImpactWidth(contacts.filter((contact) => contact.box.min.x === frontX));
        }
        return contacts;
    }
    limitContactsToImpactWidth(contacts) {
        ...
        const exposed = contacts.filter((contact) => !this.isCoveredFromFront(contact));
        ...
        const isVerticalHit = rotation === Rotation_1.Rotation.Up || rotation === Rotation_1.Rotation.Down;
        return contiguous
            .slice()
            .sort((a, b) => {
    ...
    isCoveredFromFront(contact) {
        ...
        for (const collider of this.collisionSystem.getStaticColliders()) {
            ...
            if (!object.tags.includes(Tag_1.Tag.Wall) || object.tags.includes(Tag_1.Tag.Border)) {
                continue;
            }
            const other = collider.getBox();
            if (rotation === Rotation_1.Rotation.Up) {
                // Shooter is below; the candidate's exposed face is box.max.y.
                if (other.min.x < box.max.x &&
    ...
    filterToContiguousGroup(contacts) {
        ...
        const axisRange = (contact) => isVerticalHit
        ...
        const touches = (a, b) => a[1] === b[0] || b[1] === a[0];
        ...
        while (addedAny) {
            ...
            for (let i = remaining.length - 1; i >= 0; i -= 1) {
                ...
                const touchesIncluded = included.some((contact) => touches(axisRange(contact), candidateRange));
                if (touchesIncluded) {
...
exports.TerrainTileDestroyer = TerrainTileDestroyer;
