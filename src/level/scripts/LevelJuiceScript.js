...
exports.LevelJuiceScript = void 0;
...
class LevelJuiceScript extends LevelScript_1.LevelScript {
    constructor() {
        super(...arguments);
    ...
    update(updateArgs) {
        ...
        for (const child of this.world.field.children) {
            if (!(child instanceof gameObjects_1.Tank)) {
                continue;
            }
            const tank = child;
    ...
    emitTreadDust(tank) {
        ...
        if (last === undefined) {
            this.lastDustAt.set(tank, new core_1.Vector(center.x, center.y));
            return;
        }
        ...
        if (rotation === game_1.Rotation.Up) {
            dirY = -1;
        }
        else if (rotation === game_1.Rotation.Down) {
            dirY = 1;
        }
        else if (rotation === game_1.Rotation.Left) {
            dirX = -1;
        }
        else if (rotation === game_1.Rotation.Right) {
            dirX = 1;
        }
...
exports.LevelJuiceScript = LevelJuiceScript;
