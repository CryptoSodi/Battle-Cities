...
interface XStatus {
  connected?: boolean;
  follows?: boolean;
  repostTask?: SocialTask | null;
  commentTask?: SocialTask | null;
}
...
export class SocialsWebUi {
  ...
  public mount(): void {
    ...
    host.hidden = false;
    this.render();
    window.addEventListener('focus', () => void this.refresh(false), {
  ...
  private async refresh(verifyReady: boolean): Promise<void> {
    ...
    this.loading = true;
    this.status = verifyReady
      ? 'VERIFYING SOCIAL TASKS...'
      : 'CHECKING SOCIAL TASKS...';
    this.render();
    try {
      ...
      const [xResponse, discordResponse] = await Promise.all([
        ...
        apiFetch('/api/integrations/discord/verification', {
          cache: 'no-store',
        }),
      ]);
  ...
  private async verifyReadyTasks(): Promise<void> {
    if (
      this.xStatus.connected &&
      !this.xStatus.follows &&
      this.ready(FOLLOW_READY) === '1'
    ) {
      const response = await apiFetch('/api/integrations/x/verify-follow', {
        method: 'POST',
      });
    ...
    if (repost && !repost.claimed && this.ready(REPOST_READY) === repost.id) {
      const response = await apiFetch('/api/integrations/x/verify-repost', {
        method: 'POST',
      });
      const result = await response.json().catch(() => ({}));
    ...
    if (
      comment &&
      !comment.claimed &&
      this.ready(COMMENT_READY) === comment.id
    ) {
      const response = await apiFetch('/api/integrations/x/verify-comment', {
        method: 'POST',
      });
      const result = await response.json().catch(() => ({}));
  ...
  private render(): void {
    ...
    const preferred =
      this.buttons.find(
        (button) => button.dataset.socialKey === this.lastFocusKey,
      ) ||
      this.buttons.find((button) => button.dataset.socialAction === 'website');
  ...
  private taskCard(
    ...
    if (!this.xStatus.connected || !this.xStatus.follows || !task)
      return this.card(
        key,
        title,
        mark,
  ...
  private bind(): void {
    ...
    this.buttons.forEach((button) => {
      ...
      button.addEventListener(
        'focus',
        () => {
          this.lastFocusKey = button.dataset.socialKey || this.lastFocusKey;
    ...
    this.host
      .querySelector('[data-social-refresh]')
      ?.addEventListener('click', () => void this.refresh(true), { signal });
    this.host
      .querySelectorAll<HTMLButtonElement>('[data-social-action]')
      .forEach((button) =>
        button.addEventListener(
          'click',
          () => void this.act(button.dataset.socialAction || ''),
          { signal },
        ),
      );
  }
  private async act(action: string): Promise<void> {
    if (action === 'website') this.open('https://battlecities.com');
      ...
      this.open('https://www.instagram.com/battlecitieshq');
    else if (action === 'x-follow') await this.actFollow();
    else if (action === 'x-repost')
      await this.actTask(
        this.xStatus.repostTask,
        REPOST_READY,
        'https://x.com/intent/retweet?tweet_id=',
        '/api/integrations/x/verify-repost',
      );
    else if (action === 'x-comment')
      await this.actTask(
        this.xStatus.commentTask,
        COMMENT_READY,
        'https://x.com/intent/tweet?in_reply_to=',
        '/api/integrations/x/verify-comment',
      );
    else if (action === 'discord') await this.actDiscord();
  }
  private async actFollow(): Promise<void> {
    if (!this.xStatus.connected) {
      this.open(getApiUrl('/api/integrations/x/oauth/start'));
      return;
    }
    if (this.xStatus.follows) {
      this.status = 'X FOLLOW ALREADY VERIFIED';
      this.render();
      return;
    }
    if (this.ready(FOLLOW_READY) === '1') {
      await this.refresh(true);
      return;
    }
    this.setReady(FOLLOW_READY, '1');
    this.open('https://x.com/BattleCitiesHQ');
    this.status = 'FOLLOW ON X, THEN PRESS REFRESH';
    this.render();
  }
  private async actTask(
    task: SocialTask | null | undefined,
    storageKey: string,
    url: string,
    endpoint: string,
    ...
    if (this.ready(storageKey) === task.id) {
      this.status = 'VERIFYING SOCIAL TASK...';
      this.render();
      await apiFetch(endpoint, { method: 'POST' }).catch(() => null);
      await this.refresh(false);
      return;
    }
    this.setReady(storageKey, task.id);
    this.open(`${url}${encodeURIComponent(task.postId)}`);
    this.status = 'COMPLETE THE TASK, THEN PRESS REFRESH';
    this.render();
  }
  private async actDiscord(): Promise<void> {
    if (this.discord.rewardClaimed) {
      this.status = 'DISCORD REWARD ALREADY CLAIMED';
      this.render();
      return;
    }
    if (this.discord.verified) {
      this.status = 'CLAIMING DISCORD FUEL...';
      this.render();
      await apiFetch('/api/integrations/discord/claim-reward', {
        method: 'POST',
      }).catch(() => null);
      await this.refresh(false);
      return;
    }
    this.open(getApiUrl('/api/integrations/discord/oauth/start'));
    this.status = 'VERIFY IN DISCORD, THEN PRESS REFRESH';
    this.render();
  }
  private xFollowDetail(): string {
    if (!this.xStatus.connected) return 'CONNECT X TO BEGIN';
    if (this.xStatus.follows) return 'FOLLOW VERIFIED · 5 FUEL SECURED';
    return this.ready(FOLLOW_READY) === '1'
      ? 'READY FOR LIVE VERIFICATION'
      : 'FOLLOW @BATTLECITIESHQ';
  }
  private xFollowAction(): string {
    if (!this.xStatus.connected) return 'CONNECT X';
    if (this.xStatus.follows) return 'FOLLOWED';
    return this.ready(FOLLOW_READY) === '1' ? 'VERIFY FOLLOW' : 'FOLLOW ON X';
  }
  private discordDetail(): string {
    if (!this.discord.authenticated) return 'LOGIN REQUIRED';
    if (this.discord.rewardClaimed) return 'COMPLETED · 5 FUEL SECURED';
    if (this.discord.verified) return 'MEMBERSHIP VERIFIED';
    return 'JOIN AND VERIFY MEMBERSHIP';
  }
  private discordAction(): string {
    if (this.discord.rewardClaimed) return 'VERIFIED';
    if (this.discord.verified) return 'CLAIM FUEL';
    return 'JOIN & VERIFY';
  }
  ...
  private ready(key: string): string {
    try {
  ...
  private setReady(key: string, value: string): void {
    try {
