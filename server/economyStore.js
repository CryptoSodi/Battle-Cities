...
async function ensureSchema() {
  await getPgPool().query(`
    ...
    CREATE INDEX IF NOT EXISTS battlecity_economy_accounts_provider_idx
      ON ${TABLE_NAME} (provider, updated_at DESC);
  `);
}
...
async function appendPurchaseLedgerEntries(
  ...
  try {
    ...
    // The purchase itself already committed; a ledger write failure must not
    // fail the purchase response. Postgres mode logs nothing here on purpose.
  }
}
